...
function Router() {
  ...
  const renderPage = () => {
    ...
    // Default : jeu (accessible sans compte)
    return <GameApp />;
  };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        ...
        <div style={{ border: '3px solid #1E1A12', padding: '2rem 3rem', textAlign: 'center', boxShadow: '6px 6px 0 #C8803A', background: '#F5EDD6', maxWidth: '360px', width: '100%' }}>
          {!isAuthenticated && (
            <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.62rem', color: '#8A7E65', letterSpacing: '0.1em', marginBottom: '16px', lineHeight: 1.6 }}>
              <a href="#/login" style={{ color: '#C8803A', textDecoration: 'underline' }}>Connectez-vous</a> pour sauvegarder vos parties et apparaître au classement.
            </p>
          )}
    ...
    <div style={{ minHeight: '100vh', padding: '1.5rem', maxWidth: '1280px', margin: '0 auto' }}>
      <header style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1.5rem', borderBottom: '3px solid #1E1A12', paddingBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '16px' }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Édition 1972</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: '1.5rem', alignItems: 'start' }}>
        <div>
          <Board gameState={gameState} placements={wordPlacements} onDropTile={handleDropTile} onMoveTile={handleMoveTile} onReturnTile={handleReturnTile} />
          <Legend />
        </div>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
            <RetroButton onClick={handleValidateWord} disabled={wordPlacements.length === 0 || isSwapMode} variant="primary" fullWidth>Valider ({wordPlacements.length})</RetroButton>
            <RetroButton onClick={handlePassTurn} disabled={isSwapMode || wordPlacements.length > 0} variant="default" fullWidth>Passer le tour</RetroButton>
            <RetroButton onClick={handleShuffleRack} disabled={isSwapMode || wordPlacements.length > 0} variant="tobacco" fullWidth>Mélanger</RetroButton>
          </div>
      ...
      <div style={{ marginTop: '1.5rem' }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.12em', textTransform: 'uppercase', textAlign: 'center', marginBottom: '10px' }}>
          {isSwapMode ? `${selectedTilesToSwap.length} tuile(s) sélectionnée(s)` : 'Glissez vos lettres sur le plateau'}
        </div>
        <TileRack tiles={rackTilesForDisplay} playerId={currentPlayerId} onTileClick={isSwapMode ? toggleTileForSwap : undefined} selectedTiles={selectedTilesToSwap} />
      </div>
    </div>
  );
}
...
const RetroButton = ({ onClick, disabled, children, variant = 'default', fullWidth = false }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ width: fullWidth ? '100%' : 'auto', background: v.bg, color: disabled ? '#B0A080' : v.color, border: `2px solid ${disabled ? '#C8C0A8' : v.border}`, fontFamily: "'DM Mono', monospace", fontSize: '0.7rem', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 14px', borderRadius: '2px', cursor: disabled ? 'not-allowed' : 'pointer', transition: 'background 0.12s, color 0.12s', boxShadow: disabled ? 'none' : `3px 3px 0 ${v.border}` }}
...
const ScorePreview = ({ score, count }) => (
  <div style={{ background: '#1E1A12', border: '2px solid #C8A830', borderRadius: '2px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '3px 3px 0 #8A6820' }}>
    <div>
      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.6rem', color: '#8A7E65', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '2px' }}>Score provisoire</div>
      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#6B5E45' }}>{count} tuile{count !== 1 ? 's' : ''} posée{count !== 1 ? 's' : ''}</div>
    </div>
    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '2rem', fontWeight: 700, color: '#C8A830', lineHeight: 1 }}>{score}</div>
  </div>
);
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
      {items.map(item => (
        <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.05em' }}>{item.label}</span>
        </div>
      ))}
    </div>
  );

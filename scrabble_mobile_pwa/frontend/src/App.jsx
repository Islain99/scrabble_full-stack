...
/* ── Retro Button ─────────────────────────────────────────────── */
const RetroButton = ({ onClick, disabled, children, variant = 'default', fullWidth = false, compact = false }) => {
  const variants = {
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: compact ? '9px 8px' : '8px 14px',
        borderRadius: '2px',
...
const Divider = ({ label }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '14px 0 10px' }}>
    ...
    {label && (
      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
        {label}
      </span>
    )}
    <div style={{ flex: 1, height: '1px', background: '#C8A830', opacity: 0.4 }} />
...
const ScorePreview = ({ score, count }) => (
  <div style={{
    ...
    alignItems: 'center', boxShadow: '3px 3px 0 #8A6820',
  }}>
    <div>
      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '2px' }}>
        Score provisoire
      </div>
      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.62rem', color: '#6B5E45', letterSpacing: '0.05em' }}>
        {count} tuile{count !== 1 ? 's' : ''} posée{count !== 1 ? 's' : ''}
      </div>
    </div>
    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '1.9rem', fontWeight: 700, color: '#C8A830', lineHeight: 1 }}>
      {score}
    </div>
  </div>
);
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px', justifyContent: 'center' }}>
      {items.map(item => (
        <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.05em' }}>
            {item.label}
          </span>
        </div>
      ))}
...
function App() {
  ...
  const [selectedRackIndex, setSelectedRackIndex] = useState(null); // pose au toucher
  ...
  const activePlayerId = gameState ? gameState.players[gameState.current_player_index].id : 0;

  // Réinitialise la sélection de pose quand on change de tour
  useEffect(() => { setSelectedRackIndex(null); }, [activePlayerId]);
  ...
  /* ── Interaction TAP-TAP ─────────────────────────────────── */
  const handleSelectRackTile = (rackIndex) => {
    setSelectedTilesToSwap([]); // on sort du mode échange
    setSelectedRackIndex(prev => (prev === rackIndex ? null : rackIndex));
  };

  const handleCellTap = (r, c) => {
    if (selectedRackIndex === null) return;
    handleDropTile(selectedRackIndex, r, c);
    setSelectedRackIndex(null);
  };
  ...
  const toggleTileForSwap = (letter) => {
    setSelectedRackIndex(null);
    setSelectedTilesToSwap(prev => (prev.includes(letter) ? prev.filter(l => l !== letter) : [...prev, letter]));
  };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem', maxWidth: '480px' }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(3rem, 14vw, 6rem)', fontWeight: 900, color: '#1E1A12', lineHeight: 0.9, letterSpacing: '-0.04em', margin: '0 0 8px' }}>
            SCRABBLE
          </h1>
        ...
        <div style={{ border: '3px solid #1E1A12', padding: '2rem', textAlign: 'center', boxShadow: '6px 6px 0 #C8803A', background: '#F5EDD6', maxWidth: '360px', width: '100%' }}>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.7rem', color: '#8A7E65', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '20px' }}>
            Joueur vs IA
          </p>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ border: '3px solid #1E1A12', padding: '2.5rem', background: '#F5EDD6', boxShadow: '8px 8px 0 #C8803A', maxWidth: '400px', width: '100%', textAlign: 'center' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '12px' }}>
            Partie Terminée
          </div>
          ...
          <p style={{ fontFamily: "'Libre Baskerville', serif", fontStyle: 'italic', color: '#5E6B3A', margin: '0 0 24px' }}>
  ...
  const rackBlock = (
    <TileRack
      ...
      onRackTileTap={handleSelectRackTile}
    />
  );

  const hint = isSwapMode
    ? `${selectedTilesToSwap.length} tuile(s) — touchez à nouveau pour désélectionner`
    : selectedRackIndex !== null
    ? 'Touchez une case du plateau pour poser la lettre'
    : 'Touchez une lettre, puis une case du plateau';
  ...
  if (isMobile) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        {/* Contenu scrollable */}
        <div style={{ flex: 1, padding: '12px 10px 8px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {/* Header compact */}
          <header style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', borderBottom: '2px solid #1E1A12', paddingBottom: '8px' }}>
            <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '1.6rem', fontWeight: 900, color: '#1E1A12', letterSpacing: '-0.04em', margin: 0 }}>
              SCRABBLE
            </h1>
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.6rem', color: '#8A7E65', letterSpacing: '0.05em' }}>
              Tour → <span style={{ color: '#C8803A', fontWeight: 500 }}>{gameState.players[gameState.current_player_index]?.name}</span>
            </span>
          </header>
          ...
          <Board
            ...
            isPlacing={selectedRackIndex !== null}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <RetroButton onClick={handleValidateWord} disabled={wordPlacements.length === 0 || isSwapMode} variant="primary" fullWidth compact>
              Valider ({wordPlacements.length})
            </RetroButton>
            <RetroButton onClick={handlePassTurn} disabled={isSwapMode || wordPlacements.length > 0} variant="default" fullWidth compact>
              Passer
            </RetroButton>
            <RetroButton onClick={handleShuffleRack} disabled={isSwapMode || wordPlacements.length > 0} variant="tobacco" fullWidth compact>
              Mélanger
            </RetroButton>
            <RetroButton onClick={handleSwapTiles} disabled={selectedTilesToSwap.length === 0 || wordPlacements.length > 0} variant="danger" fullWidth compact>
              Échanger {selectedTilesToSwap.length > 0 ? `(${selectedTilesToSwap.length})` : ''}
            </RetroButton>
          </div>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.56rem', color: '#8A7E65', letterSpacing: '0.04em', textAlign: 'center', margin: '2px 0', lineHeight: 1.5 }}>
            Pour échanger : touchez les lettres du rack, puis « Échanger ».
          </p>
        </div>

        {/* Rack collant en bas */}
        <div className="mobile-rack-bar">
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.06em', textTransform: 'uppercase', textAlign: 'center', marginBottom: '8px' }}>
            {hint}
          </div>
          {rackBlock}
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '1.5rem', maxWidth: '1280px', margin: '0 auto' }}>
      <header style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1.5rem', borderBottom: '3px solid #1E1A12', paddingBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '16px' }}>
          <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(1.8rem, 5vw, 2.8rem)', fontWeight: 900, color: '#1E1A12', letterSpacing: '-0.04em', margin: 0 }}>
            SCRABBLE
          </h1>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
            Édition 1972
          </span>
        </div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.1em' }}>
          Tour de → <span style={{ color: '#C8803A', fontWeight: 500 }}>{gameState.players[gameState.current_player_index]?.name}</span>
        </div>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
            <RetroButton onClick={handleValidateWord} disabled={wordPlacements.length === 0 || isSwapMode} variant="primary" fullWidth>
              Valider ({wordPlacements.length})
            </RetroButton>
            <RetroButton onClick={handlePassTurn} disabled={isSwapMode || wordPlacements.length > 0} variant="default" fullWidth>
              Passer le tour
            </RetroButton>
            <RetroButton onClick={handleShuffleRack} disabled={isSwapMode || wordPlacements.length > 0} variant="tobacco" fullWidth>
              Mélanger
            </RetroButton>
          </div>
          <Divider label="Échange" />
          <RetroButton onClick={handleSwapTiles} disabled={selectedTilesToSwap.length === 0 || wordPlacements.length > 0} variant="danger" fullWidth>
            Échanger {selectedTilesToSwap.length > 0 ? `(${selectedTilesToSwap.length})` : ''}
          </RetroButton>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.58rem', color: '#8A7E65', letterSpacing: '0.06em', marginTop: '6px', lineHeight: 1.5 }}>
            Touchez une lettre du rack pour la sélectionner, puis une case. (Le glisser-déposer fonctionne aussi.)
          </p>
        </aside>
      </div>

      <div style={{ marginTop: '1.5rem' }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', color: '#8A7E65', letterSpacing: '0.12em', textTransform: 'uppercase', textAlign: 'center', marginBottom: '10px' }}>
          {hint}
        </div>
        {rackBlock}
      </div>
